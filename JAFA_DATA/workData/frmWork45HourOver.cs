    ...
    public partial class frm45HourOver : Form
    {
        ...
        private void GridViewShowData(DataGridView g, int sYY, int sMM, int sSel)
        {
            try
            {
                ...
                string sYYMM = (sYY * 100 + sMM).ToString();
                ...
                {
                    //MessageBox.Show(t.対象職員コード + ":" + t.対象職員名);

                    if (dts.社員マスター.Any(a => a.職員コード == Utility.StrtoInt(t.対象職員コード)))
                    {
                        var s = dts.社員マスター.Single(a => a.職員コード == Utility.StrtoInt(t.対象職員コード));
